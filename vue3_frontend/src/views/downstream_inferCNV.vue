...
const submit_inferCNV = () => {
  ...
  formData.append("num_threads", num_threads.value)
  formData.append("cutoff", cutoff.value)
  formData.append("ref_group_names", ref_group_names.value)

  axios.post("/downstream/scRNA/inferCNV", formData, {
    headers: {
  ...
  <div v-show="store.displayOther">
    ...
    <div class="parameters">
      ...
      <div class="EnterID">
          ...
          <p>Enter data index in "for downstream analysis" to do :
            <input type="text" v-model="data_index">
          </p>
          <h3>inferCNV parameters</h3>
          <p>
            Enter reference cell type names:
            <input type="text" v-model="ref_group_names">
          </p>
          <p>
            Enter cutoff value:
            <input type="text" v-model="cutoff">
          </p>
          <p>
            Enter number of threads to use:
            <input type="text" v-model="num_threads">
          </p>
          <p>
            <input type="button" value="submit" @click="submit_inferCNV()">
          </p>
          <br>
      </div>
