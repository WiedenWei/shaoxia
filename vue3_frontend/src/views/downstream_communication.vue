...
let data_index = ref("")
let celltypeList = ref<Array<string>>([])
...
const addOne = () => {
  celltypeList.value.push("")
}

const deleteOne = (i: number) => {
  celltypeList.value.splice(i,1)
}
...
const submit_communication = () => {
  ...
  formData.append("celltypes", celltypeList.value.toString())// 逗号分隔的字符串

  axios.post("/downstream/scRNA/communication", formData, {
    headers: {
...
<template>
  ...
  <div v-show="store.displayOther">
    ...
    <div class="header">
      <h1>Downstream</h1>
      <h2>scRNA-seq data | cell communication</h2>
    </div>
    ...
    <div class="parameters">
      ...
      <div class="EnterID">
        ...
        <p>Enter data index in "for downstream analysis" to do cell communication ananlysis:
          <input type="text" v-model="data_index">
        </p>
        <h3>Enter cell types for visualizing interactions among them:</h3>
        <div v-for="(v, i) in celltypeList" :key="i">
          <p>
            cell type: <input type="text" v-model="celltypeList[i]">
            &nbsp;
        ...
        <br>
        <p>
          <input type="button" value="submit" @click="submit_communication()">
        </p>
        <br>
      </div>
